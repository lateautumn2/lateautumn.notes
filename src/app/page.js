      ...
      <div className="lg:grid lg:grid-cols-6 lg:gap-4 pt-12 max-w-6xl">
        <div className="max-w-xl mx-auto col-span-5">
          ...
          <Link href="/blog" passHref>
            <p className="text-right text-sm text-zinc-500 hover:text-zinc-900 hover:underline transition duration-300">Read More →</p>
          </Link>
        </div>
        <div className="col-span-1 max-w-lg mx-auto">
          <Image src={siteMetadata.image} alt="Avatar" width="100" height="100" className="rounded-full max-w-md mx-auto shadow mt-4" />
          <h2 className="font-semibold py-4 text-center">{siteMetadata.author}</h2>

          <p className="py-4 mt-2 text-center">“ {siteMetadata.say} ”</p>
          <Link href="/about" passHref>
            <p className="text-right text-sm pt-2 text-zinc-500 hover:text-zinc-900 hover:underline transition duration-300">About More →</p>
          </Link>
        </div>
      </div>
    </div>
